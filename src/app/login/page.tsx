...
import { signInWithGoogle, signInWithGoogleRedirect, auth } from "@/lib/firebase/auth";
...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      if (inAppBrowser) {
        // Use redirect flow for Instagram/in-app browsers
        await signInWithGoogleRedirect();
        // Don't setIsLoading(false) because the page will redirect
      } else {
        // Use popup flow for standard browsers
      ...
      >
        <div className="w-full max-w-md flex flex-col items-center text-center">
          ...
          <motion.div variants={fadeUpVariants} className="w-full max-w-[320px]">
            {inAppBrowser ? (
              <div className="bg-orange-50 border border-orange-200 text-orange-800 text-sm p-4 rounded-xl shadow-sm text-left">
                ...
                <p className="opacity-90">
                  Google Sign-In is blocked inside this app's browser (like Instagram or Facebook). Please open this link in your device's default browser (Safari or Chrome).
                </p>
              </div>
            ) : (
              <motion.button
